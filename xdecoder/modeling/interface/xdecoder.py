...
class XDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        ...
        results, frame_query = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0], task=task)
        ...
        for i in range(self.num_layers):
            ...
            results, frame_query = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], layer_id=i, task=task)
            attn_mask = results["attn_mask"]
            frame_queries.append(frame_query[:,self.num_queries:2*self.num_queries-1])
            predictions_class.append(results["outputs_class"][:,self.num_queries:2*self.num_queries-1])
            predictions_mask.append(results["outputs_mask"][:,self.num_queries:2*self.num_queries-1])
            predictions_bbox.append(results["outputs_bbox"])
            predictions_caption.append(results["outputs_caption"][:,self.num_queries:2*self.num_queries-1])
            predictions_captioning.append(results["outputs_captionting"])
        ...
        if task == 'vlp':
            ...
            frame_queries = torch.stack(frame_queries[-3:]) # same as mask2former
            out = {
